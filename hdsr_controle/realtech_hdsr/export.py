...
class ShpResponder(object):
    def __init__(self, queryset,  geo_field=None, proj_transform=None,file_name='shp_download'):
        self.queryset = queryset
        self.geo_field = geo_field
        self.proj_transform = proj_transform
        self.file_name = smart_str(os.path.basename(file_name))
        self.distPath = os.path.dirname(file_name) 
        tmp = self.write_shapefile_to_tmp_file(self.queryset)
        self.zip_response(shapefile_distpath=self.distPath, shapefile_tmppath= tmp, file_name= self.file_name)
    ...
    def zip_response(self,shapefile_distpath,shapefile_tmppath,file_name):
        
        zipfilename = '%s.zip' %(os.path.join( shapefile_distpath,file_name.replace('.shp','') ))
        zip = zipfile.ZipFile(zipfilename, 'w', zipfile.ZIP_DEFLATED)
        files = ['shp','shx','prj','dbf']
        for item in files:
            filename = '%s.%s' % (shapefile_tmppath.replace('.shp','') , item)
            zip.write(filename, arcname='%s.%s' % (file_name, item))
        zip.close()
