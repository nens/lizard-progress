...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """For each upload log, find which activity probably belongs to its
        combination of project, uploading_organization and mtype, and
        set that. If there are more than one, pick the first.
        If there are none, CRASH - that should be impossible.
        """

        for uploadlog in (
                orm['lizard_progress.UploadLog'].objects.select_related()):
            activities = list(orm['lizard_progress.Activity'].objects.filter(
                project_id=uploadlog.project_id,
                contractor_id=uploadlog.uploading_organization_id,
                measurement_type=uploadlog.mtype_id)[:1])

            if activities:
                uploadlog.activity = activities[0]
                uploadlog.save()
            else:
                raise ValueError(
                    "Could not find activity for {}.".format(uploadlog))
