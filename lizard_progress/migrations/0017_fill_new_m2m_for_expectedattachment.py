...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        expected_attachments = orm[
            'lizard_progress.ExpectedAttachment'].objects.all()

        for attachment in expected_attachments:
            for location in attachment.location_set.all():
                measurements = list(location.measurement_set.filter(
                    data={'filetype': 'ribx'}))

                # We don't know to which of the measurements the attachment
                # belongs, so we take the last one.
                measurement = measurements[-1]
                measurement.expected_attachments.add(attachment)
