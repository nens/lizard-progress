...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        uploaded_attachments = orm[
            'lizard_progress.ExpectedAttachment'].objects.filter(uploaded=True)

        for attachment in uploaded_attachments:
            # This is an expected attachment that was later
            # uploaded. It is attached to one or more measurements
            # that announced that this file was coming, and there is a
            # measurement for the uploaded attachment itself at the
            # same location, that needs to get its "parent" link set.
            for measurement in attachment.measurement_set.all():
                location = measurement.location
                for attach_measurement in location.measurement_set.all():
                    # Because this isn't a real Measurement but a South
                    # construct, we can't use our own properties on it,
                    # have to calculate basename by hand.
                    attach_base_name = os.path.basename(
                        attach_measurement.filename)
                    if attach_measurement.data.get('filetype') != 'media':
                        # Not the attachment -- normal
                        continue
                    if attach_base_name != attachment.filename:
                        # Something strange, let's skip
                        print(
                            "Attachment found, but {} and {} aren't equal."
                            .format(attach_base_name, attachment.filename))
                        continue
                    if attach_measurement.parent is not None:
                        # It was already set?
                        print(
                            "Attachment {} had its parent already set."
                            .format(attach_measurement))
                        continue
                    ...
                    print(
                        "Setting '{}'.parent to '{}'."
                        .format(attach_base_name, attachment.filename))
                    attach_measurement.parent = measurement
                    attach_measurement.save()
