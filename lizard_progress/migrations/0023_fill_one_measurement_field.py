...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Fill the one_measurement_uploaded field of all Locations
        # with True if a measurement exists.
        locations_with_measurements = set(
            orm['lizard_progress.Measurement'].objects.values_list(
                'location_id', flat=True))

        for location in orm['lizard_progress.Location'].objects.all():
            new_value = location.id in locations_with_measurements
            if location.one_measurement_uploaded != new_value:
                location.one_measurement_uploaded = new_value
                location.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        # Not needed; going backwards, we simply don't care what the
        # value of one_measurement is.
