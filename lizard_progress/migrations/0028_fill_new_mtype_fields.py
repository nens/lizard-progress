...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # The 'laboratorium_csv' type doesn't need locations or
        # scheduled measurements
        mtype = orm.AvailableMeasurementType.objects.get(
            slug='laboratorium_csv')
        mtype.needs_predefined_locations = False
        mtype.needs_scheduled_measurements = False
        mtype.save()

        # Initial descriptions
        descriptions = {
            'dwarsprofiel': """MET files met dwarsprofielen. Voor dit type moet in de volgende stap een shapefile met in te meten locaties geupload worden. """,
            'oeverfoto': """Twee foto's, van beide oevers een foto. De locaties moeten in de volgende stap aangegeven worden met een shapefile, en voor een locatie met een id 'xxx' moeten de foto's als bestandsnaam 'xxx_L.jpg' en 'xxx_R.jpg' hebben. Er is een controle van de afstand tussen het punt in de shapefile en de GPS gegevens in de foto.""",
            'oeverkenmerk': """CSV files waarin de inrichting van de oever aangegeven kan worden.""",
            'foto': """1 foto. De locaties worden vooraf aangegeven in een shapefile. De bestandsnaam van de foto op locatie 'xxxx' moet 'xxxx.jpg' zijn. Er is een controle van de afstand tussen het punt in de shapefile en de GPS gegevens in de foto.""",
            'meting': """CSV files met metingen van peilschalen.""",
            'laboratorium_csv': """CSV files met resultaten van laboratorium analyses.""",
            }

        for slug, description in descriptions.items():
            mtype = orm.AvailableMeasurementType.objects.get(
                slug=slug)
            mtype.description = description
            mtype.save()
    ...
    models = {
        ...
        'auth.user': {
            ...
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 9, 6, 10, 42, 9, 488149)'}),
            ...
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 9, 6, 10, 42, 9, 488067)'}),
