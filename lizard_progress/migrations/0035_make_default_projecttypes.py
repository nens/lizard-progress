...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for organization in orm['lizard_progress.Organization'].objects.all():
            if organization.projecttype_set.all().count() == 0:
                projecttype = (
                    orm['lizard_progress.ProjectType'].objects.create(
                        name='Project', description='Project',
                        organization=organization, default=True))
            else:
                projecttype = organization.projecttype_set.get(default=True)

            for project in orm['lizard_progress.Project'].objects.filter(
                organization=organization, project_type__isnull=True):
