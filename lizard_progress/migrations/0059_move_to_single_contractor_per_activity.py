...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Split activities into several activities, if needed."""

        for activity in orm['lizard_progress.Activity'].objects.all():
            pairs = [
                (organization, measurement_type)
                for organization in activity.contractors.all()
                for measurement_type in activity.measurement_types.all()
                ]

            for i, (organization, measurement_type) in enumerate(pairs):
                if i == 0:
                    # Update the current activity
                    current_activity = activity
                else:
                    # Make a new activity, relate relevant scheduled
                    # measurements and uploaded files to it.
                    current_activity = orm[
                        'lizard_progress.Activity'].objects.create(
                        project=activity.project,
                        name="{} {}".format(activity.name, i))

                    for location in activity.location_set.all():
                        scheduleds = list(
                            location.scheduledmeasurement_set.filter(
                                available_measurement_type=measurement_type,
                                organization=organization))
                        if scheduleds:
                            # Copy location
                            newloc = (
                                orm['lizard_progress.Location'].objects.create(
                                    activity=current_activity,
                                    location_code=location.location_code,
                                    the_geom=location.the_geom,
                                    information=location.information))
                            for scheduled in scheduleds:
                                scheduled.location = newloc
                                scheduled.save()

                    for uploaded in activity.uploadedfile_set.filter(
                            organization=organization,
                            mtype=measurement_type):
                        uploaded.activity = current_activity
                        uploaded.save()

                current_activity.measurement_type = measurement_type
                current_activity.contractor = organization
