...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing unique constraint on 'ExportRun', fields ['project', 'organization', 'measurement_type', 'exporttype']
        db.delete_unique(u'lizard_progress_exportrun', ['project_id', 'organization_id', 'measurement_type_id', 'exporttype'])

        # Deleting field 'ExportRun.measurement_type'
        db.delete_column(u'lizard_progress_exportrun', 'measurement_type_id')

        # Deleting field 'ExportRun.project'
        db.delete_column(u'lizard_progress_exportrun', 'project_id')

        # Deleting field 'ExportRun.organization'
        db.delete_column(u'lizard_progress_exportrun', 'organization_id')

        # Adding unique constraint on 'ExportRun', fields ['exporttype', 'activity']
        db.create_unique(u'lizard_progress_exportrun', ['exporttype', 'activity_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'ExportRun', fields ['exporttype', 'activity']
        db.delete_unique(u'lizard_progress_exportrun', ['exporttype', 'activity_id'])


        # User chose to not deal with backwards NULL issues for 'ExportRun.measurement_type'
        raise RuntimeError("Cannot reverse this migration. 'ExportRun.measurement_type' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'ExportRun.project'
        raise RuntimeError("Cannot reverse this migration. 'ExportRun.project' and its values cannot be restored.")
        # Adding field 'ExportRun.organization'
        db.add_column(u'lizard_progress_exportrun', 'organization',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['lizard_progress.Organization'], null=True),
                      keep_default=False)

        # Adding unique constraint on 'ExportRun', fields ['project', 'organization', 'measurement_type', 'exporttype']
        db.create_unique(u'lizard_progress_exportrun', ['project_id', 'organization_id', 'measurement_type_id', 'exporttype'])
